...
class Challenge(ChallengeBase):
    ...
    def generate_functional_test(self) -> List[Dict[str, Any]]:
        ...
        # Small test cases
        test_cases.append({
            "input": torch.tensor([255.0, 0.0, 0.0], device="cuda", dtype=torch.float32),  # red pixel
        ...
        test_cases.append({
            "input": torch.tensor([0.0, 255.0, 0.0], device="cuda", dtype=torch.float32),  # green pixel
            "output": torch.zeros(1, device="cuda", dtype=torch.float32),
            "width": 1,
            "height": 1,
        })
        ...
        test_cases.append({
            "input": torch.tensor([0.0, 0.0, 255.0], device="cuda", dtype=torch.float32),  # blue pixel
            "output": torch.zeros(1, device="cuda", dtype=torch.float32),
            "width": 1,
            "height": 1,
        })
        ...
        test_cases.append({
            "input": torch.tensor([
                100.0, 150.0, 200.0,  # mixed color 1
                50.0, 75.0, 100.0,    # mixed color 2
                200.0, 100.0, 50.0,   # mixed color 3
                75.0, 125.0, 175.0    # mixed color 4
            ], device="cuda", dtype=torch.float32),
        ...
        # Non-square images
        test_cases.append({
            "input": torch.randint(0, 256, (2 * 3 * 3,), device="cuda", dtype=torch.float32),  # 2x3 image
        ...
        test_cases.append({
            "input": torch.randint(0, 256, (3 * 2 * 3,), device="cuda", dtype=torch.float32),  # 3x2 image
